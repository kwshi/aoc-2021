...
@ft.lru_cache(None)
def align(a: int, b: int):
    for pa, pb in it.product(scans[a], scans[b]):
        transforms: list[set[Pt]] = [set() for _ in range(24)]
        for p in scans[b]:
            for t, o in zip(transforms, p.orientations()):
                t.add(o + pa)
...
print("yo")
for a, b in it.combinations(scans.keys(), 2):
    align(a, b)
